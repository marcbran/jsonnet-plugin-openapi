...
func Exec(ctx context.Context, in Input) (Output, error) {
	...
	pipeline := inference.Pipeline{
		Jobs: []inference.Job{
			NewLinksFromResourcesJob(renderer),
	...
	links, err := renderer.RenderOutput("links.jsonnet",
		infrajsonnet.Binding{Name: "spec", Value: spec.JSON},
		infrajsonnet.Binding{Name: "inferred", Value: mergedLinkResults(results)},
		infrajsonnet.Binding{Name: "varsInferred", Value: results[LinkVarsJobName]},
	)
	if err != nil {
		return Output{}, err
	}
	...
	return Output{
		...
		WorkDir: workDir,
		Files: []string{
			relativeOrAbs(workDir, filepath.Join(workDir, LinksFromResourcesJobName, "results", "all.jsonnet")),
			relativeOrAbs(workDir, filepath.Join(workDir, LinksFromCollectionsJobName, "results", "all.jsonnet")),
			relativeOrAbs(workDir, filepath.Join(workDir, LinkVarsJobName, "results", "all.jsonnet")),
			out,
		},
	}, nil
}

// mergeResults concatenates two jsonnet array expressions produced by
// Store.LoadAll into a single array expression. Empty inputs are treated as [].
